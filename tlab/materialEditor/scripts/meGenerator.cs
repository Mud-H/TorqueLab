...
function MatEd::generateMapPills(%this) {
   ...
   foreach$(%map in $MatEd_TextureMaps){
      ...
      %bitmapType.command = "MaterialEditorTools.updateTextureMap(\""@%map@"\", 1);";
      ...
      %pill-->editIcon.command = "MaterialEditorTools.updateTextureMap(\""@%map@"\", 1);";
      %pill-->deleteIcon.command = "MaterialEditorTools.updateTextureMap(\""@%map@"\", 0);";
